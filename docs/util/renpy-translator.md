...
原理其实是通过 hook 游戏加载 rpa 包后的节点，将内存中的数据 dump 出来，但解出来的脚本是 rpyc ，需要再结合 unren 反编译（会自动调用），点开工具的高级选项，找到解压游戏包，你只需要把game.exe放进去即可进行解包
...
在反编译翻rpyc后就可以进行生成翻译了，此工具现提供三种生成翻译文件模式：
...
而在如今的乱世，诞生出一个不求赞助，完全开源的猛士。可谓乱杀于今世，只为大家谋福利。我难以用语言形容他的伟大，这正是互联网之初的开源精神，正因为这样，才会让世界更加美好！

同时，也担心有意之士会以此套壳加以倒卖，粗制滥造成为主流。这是不可避免的，所以还是那句话，如果人人都会汉化，那么机翻将无路可寻，这便是最初的初衷，也是这个软件最初的初衷。
